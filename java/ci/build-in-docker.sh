...
#PARALLEL_LEVEL=12  cmake -GNinja -DCMAKE_BUILD_TYPE=Release -DCMAKE_CUDA_ARCHITECTURES="70" -DCMAKE_INSTALL_PREFIX=$CONDA_PREFIX -DARROW_STATIC_LIB=ON -DUSE_NVTX=0 -DRMM_LOGGING_LEVEL=OFF -DBUILD_BENCHMARKS=OFF -DBUILD_TESTS=OFF -DCUDF_USE_ARROW_STATIC=ON -DCUDA_USE_STATIC_CUDA_RUNTIME=OFF ..
cmake .. -DCUDA_ENABLE_LINEINFO=ON -DCMAKE_CUDA_ARCHITECTURES="70" -DUSE_NVTX=$ENABLE_NVTX -DCUDF_USE_ARROW_STATIC=ON -DBoost_USE_STATIC_LIBS=ON -DBUILD_TESTS=$SKIP_CPP_TESTS -DPER_THREAD_DEFAULT_STREAM=$ENABLE_PTDS -DRMM_LOGGING_LEVEL=$RMM_LOGGING_LEVEL
