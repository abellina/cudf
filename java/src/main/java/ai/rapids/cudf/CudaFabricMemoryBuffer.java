 ...
 */
public class CudaFabricMemoryBuffer extends BaseDeviceMemoryBuffer {
  private static final Logger log = LoggerFactory.getLogger(CudaFabricMemoryBuffer.class);

  private static final class CudaFabricBufferCleaner extends MemoryBufferCleaner {
    private long address;
    private long lengthInBytes;
    private Cuda.Stream stream;

    CudaFabricBufferCleaner(long address, long lengthInBytes, Cuda.Stream stream) {
    ...
    protected synchronized boolean cleanImpl(boolean logErrorIfNotClean) {
      ...
      if (neededCleanup && logErrorIfNotClean) {
        log.error("A CUDA FABRIC BUFFER WAS LEAKED (ID: " + id + " " + Long.toHexString(origAddress) + ")");
   ...
   */
  public CudaFabricMemoryBuffer(long address, long lengthInBytes, Cuda.Stream stream) {
    super(address, lengthInBytes, new CudaFabricBufferCleaner(address, lengthInBytes, stream));
  }

  private CudaFabricMemoryBuffer(long address, long lengthInBytes, CudaFabricMemoryBuffer parent) {
   ...
   * @return the buffer
   */
  public static CudaFabricMemoryBuffer allocate(long bytes) {
   ...
   * @return the buffer
   */
  public static CudaFabricMemoryBuffer allocate(long bytes, Cuda.Stream stream) {
    long address = Cuda.allocFabric(bytes);
    return new CudaFabricMemoryBuffer(address, bytes, stream);
  }
   ...
   */
  @Override
  public synchronized final CudaFabricMemoryBuffer slice(long offset, long len) {
    addressOutOfBoundsCheck(address + offset, len, "slice");
    incRefCount();
    return new CudaFabricMemoryBuffer(getAddress() + offset, len, this);
  }
}
