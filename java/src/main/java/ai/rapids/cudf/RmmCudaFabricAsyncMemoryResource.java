 ...
 * deallocation.
 */
public class RmmCudaFabricAsyncMemoryResource implements RmmDeviceMemoryResource {
   ...
   */
  public RmmCudaFabricAsyncMemoryResource(long size, long releaseThreshold) {
    handle = Rmm.newCudaFabricAsyncMemoryResource(size, releaseThreshold);
    this.size = size;
  ...
  public void close() {
    if (handle != 0) {
      Rmm.releaseCudaFabricAsyncMemoryResource(handle);
      handle = 0;
    }
  }
