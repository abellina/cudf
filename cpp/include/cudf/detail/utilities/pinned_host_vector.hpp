...
class pinned_allocator {
 public:
  ...
  __host__ inline pointer allocate(size_type cnt, const_pointer /*hint*/ = 0)
  {
    ...
    pointer result(0);
    static_pinned_pool::get_instance()->allocate(
      reinterpret_cast<void**>(&result), cnt * sizeof(value_type));
    return result;
  }
   ...
   */
  __host__ inline void deallocate(pointer p, size_type sz){ 
    static_pinned_pool::get_instance()->free(
      p, (std::size_t)(sz * sizeof(value_type)));
  }
