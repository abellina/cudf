...
auto contiguous_copy_column_device_views(HostTableView source_view, rmm::cuda_stream_view stream)
{
  ...
  auto pinned_buffer = host_mr.allocate(views_size_bytes);
  memcpy(pinned_buffer, h_ptr, views_size_bytes);
  cudf::smart_h2d(d_ptr, pinned_buffer, views_size_bytes, stream);

  //CUDF_CUDA_TRY(cudaMemcpyAsync(d_ptr, h_ptr, views_size_bytes, cudaMemcpyDefault, stream.value()));
  stream.synchronize();
  host_mr.deallocate(pinned_buffer, views_size_bytes);
